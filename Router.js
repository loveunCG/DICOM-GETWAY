...
app.post('/executeCreateUps', function(req, res) {
	...
	let command =
		...
		req.query.hostName +
		' -f ' +
		'G:\\wamp64\\www\\RIS\\assets\\upsxml\\' +
		req.query.fileName +
		'.xml';
	console.log(command);
...
function dicomQRStudy(pacs, study_IUID, response) {
	...
	var command =
		process.env.PATH_BASE_DCM4CHE2 +
		'dcmqr -L ' +
		process.env.AETITLE_GATEWAY +
		':11114 -I ' +
		pacs.getDicomServer() +
		' -q 0020000D=' +
		study_IUID +
...
function dicomQR(pacs, request, response) {
	...
	var qPatId = typeof request.query.patId != 'undefined' ? ' -qPatientID=' + request.query.patId : '';
	var qStudyDate = typeof request.query.studyDate != 'undefined' ? ' -q StudyDate=' + request.query.studyDate : '';
...
function processResponse(outputRes, encoding, response) {
	...
	if (process.env.DEBUG_LEVEL >= process.env.DEBUG_INFO) {
	} else if (process.env.DEBUG_LEVEL == process.env.DEBUG_NONE) {
...
function identifyPattern(testStr) {
	...
	if (process.env.SHOW_REQUEST) {
		...
		patterns[process.env.QUERY_REQUEST] = new RegExp(
			'^Send Query Request #([1-9][0-9]*)/([1-9][0-9]*) using ([0-9]+(?:.[0-9]+)+)/'
		); //^Send Query Request #([1-9][0-9]*)\/([1-9][0-9]*) using ([0-9]+(?:\.[0-9]+)+)\/([[:alpha:][:space:]\/\-]+):$')
	}
	// Query Response #1:
	patterns[process.env.QUERY_RESPONSE_ROOT] = new RegExp('^Query Response #([1-9][0-9]*):$');
	// Query Response #1 for Query Request #1/3:
	patterns[process.env.QUERY_RESPONSE] = new RegExp(
		'^Query Response #([1-9][0-9]*) for Query Request #([1-9][0-9]*)/([1-9][0-9]*):$'
	);
	var matches = [];
		...
		if (matches != null) {
			...
			switch (key) {
				...
				case parseInt(process.env.QUERY_REQUEST):
					...
					element['xmlPre'] +=
						'<' + element['tag'] + " number='" + matches[1] + "' qrim='" + matches[3] + "'>\n";
					break;
				case parseInt(process.env.QUERY_RESPONSE):
					element['tag'] = 'qresponse';
					element['xmlPre'] +=
						'<' + element['tag'] + " number='" + matches[1] + "' qrequest='" + matches[2] + "'>\n";
					break;
				default:
					element['tag'] = 'dummy';
